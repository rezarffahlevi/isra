  ...
  $totalpengunjung  = mysqli_result(mysqli_query($conn, "SELECT COUNT(hits) FROM statistik"), 0); 
  ...
  $totalhits        = mysqli_result(mysqli_query($conn, "SELECT SUM(hits) FROM statistik"), 0); 
  $tothitsgbr       = mysqli_result(mysqli_query($conn, "SELECT SUM(hits) FROM statistik"), 0); 
  $bataswaktu       = time() - 300;
...
elseif ($_GET['module']=='statistik'){
  if ($_SESSION['leveluser']=='admin'){
    include "../modul/home/statistik/statistik.php";
  }
}
